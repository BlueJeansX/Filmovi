...
// pozovi div iz html-a koji će se ponavljati za svaki film iz polja objekata
document.getElementById("glavniDiv").innerHTML = `
${podaci.map(podaciFilma).join('')}
`;

// definicija funkcije čiji je zadatak dohvatiti konkretne podatke iz polja, 
//a ta funkcija zove se odmah iznad ovog komentara:

/* <img src="${film.show.image.medium}"></img> --> dohvaćanje img daje error
Cannot read properties of null at Array.map (<anonymous>) */
...
function podaciFilma(film) {
  ...
  <div class="filmic">
    ...
    <h3>${film.show.rating.average} <span>(ocjena)</span></h3>
  </div>
  `
};
...
//   const overSeven = podaci.filter(filtrirani => {
//     return filtrirani.show.rating.average > 7
//   })

//   addList(overSeven, list);
// };



// ispiši u consoli sve objekte sa ocjenom > 7
const overSeven = podaci.filter(filtrirani => {
return filtrirani.show.rating.average > 7
});
